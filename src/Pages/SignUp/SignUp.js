...
const SignUp = (props) => {
  ...
  const [values, setValues] = useState({ name: "", phone: "", birth: "", id: "", pw: "", type: undefined });
  ...
  const submitHandler = (event) => {
    ...
    if (nameIsValid && phoneIsValid && birthIsValid && idIsValid && pwIsValid && confirmationIsValid) {
      setFormIsValid(true);
    } else {
  ...
  const [childrenButtonClass, setChildrenButtonClass] = useState(classes["selected"]);
    ...
    <SignLayout>
      <form className={classes.formContainer} onSubmit={submitHandler}>
        ...
        <div className={classes.typeSelector}>
          <button type="button" className={childrenButtonClass} onClick={childrenHandler}>
            아이
          </button>
          <button type="button" className={parentsButtonClass} onClick={parentsHandler}>
            부모
          </button>
        </div>
        <Input
        ...
        />
        <input type="hidden" id="userType" name="userType" label="userType" value={userType} />
        {isError && <p className={classes.errorMessage}>회원가입에 실패했습니다.</p>}
